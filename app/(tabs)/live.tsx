...
// We accumulate readings over a 500ms window and compare peak vs baseline.
// Thresholds raised significantly to avoid false positives from minor shakes.
const BRAKE_DELTA        = 0.90;  // sudden change > 0.90G = hard brake
const TURN_DELTA         = 0.65;  // sudden change > 0.65G = sharp turn
const ACCEL_WINDOW_MS    = 500;   // wider window for peak detection
const MIRROR_CHECK_MS    = 5000;  // 5s window after a turn to check mirrors
const NO_FACE_COOLDOWN_MS = 10000; // 10s between no-face deductions
...
export default function App() {
  ...
  // Mirror check: set when a sharp turn is detected, cleared when gaze swings
  const mirrorCheckDeadline = useRef<number | null>(null);
  const lastNoFaceTime      = useRef<number>(0); // timestamp of last no-face deduction
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          if (!data.face_detected) {
            // Only warn/deduct once every 10 seconds when out of frame
            const now = Date.now();
            if (now - lastNoFaceTime.current >= NO_FACE_COOLDOWN_MS) {
              lastNoFaceTime.current = now;
              const bucket = Math.floor(secondsRef.current / 10);
              applyDeduction(`no_face_${bucket}`, 5);
              addAlert('⚠️ Face not detected');
            }
          } else if (data.drowsy) {
            const bucket = Math.floor(secondsRef.current / 8);
            applyDeduction(`drowsy_${bucket}`, 8);
            addAlert('😴 Drowsiness detected');
          } else if (data.attention_ok === false) {
            // Wider bucket = less frequent deductions for looking away
            const bucket = Math.floor(secondsRef.current / 10);
            applyDeduction(`distracted_${bucket}`, 3);
            addAlert('👀 Eyes off road');
          }

          // Mirror check: if a turn was detected, watch for a gaze swing (|gaze_x| > 0.35)
          // within the 5s window. If the user glances left/right, clear the check.
          if (mirrorCheckDeadline.current !== null) {
            const now = Date.now();
            if (data.face_detected && Math.abs(data.gaze_x ?? 0) > 0.35) {
              // Mirror glance detected — clear the check
              mirrorCheckDeadline.current = null;
            } else if (now > mirrorCheckDeadline.current) {
              // Window expired without a mirror check
              mirrorCheckDeadline.current = null;
              const bucket = Math.floor(secondsRef.current / 5);
              applyDeduction(`mirror_${bucket}`, 4);
              addAlert('🪞 No mirror check on turn');
            }
          }
        } catch (e) {
  ...
  // Reads at 50ms intervals, keeps a 500ms rolling window.
  ...
  useEffect(() => {
    ...
    const sub = Accelerometer.addListener(({ x, y, z }) => {
      ...
      // Update rolling window (keep last ~500ms = 10 samples at 50ms)
      accelWindow.current.push(magnitude);
      if (accelWindow.current.length > 10) accelWindow.current.shift();
      ...
      if (accelWindow.current.length < 10) return;
      if (now - lastAccelEvent.current < 3000) return; // 3s cooldown between events
      ...
      if (delta > BRAKE_DELTA) {
        ...
        addAlert('🛑 Hard brake detected');
      } else if (delta > TURN_DELTA) {
        lastAccelEvent.current = now;
        applyDeduction(`turn_${bucket}`, 3);
        addAlert('↩️ Sharp turn detected');
        // Start mirror check window — user has 5s to glance at mirrors
        mirrorCheckDeadline.current = now + MIRROR_CHECK_MS;
      }
    });
    return () => sub.remove();
  }, []); // runs once — uses refs for all state
  ...
  async function handleEndTrip() {
    ...
    try {
      await push(ref(db, 'trips'), {
        ...
        mirrorEvents:    [...deductions.current].filter((k) => k.startsWith('mirror')).length,
        timestamp:       Date.now(),
      });
    } catch (e) {
